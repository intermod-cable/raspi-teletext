 ...
 *
 *  FIXED = 24 pixels: the 3-byte synchronisation sequence (CS+BS) is written
 *  once in init() into the fixed preamble region and never overwritten.
 *  copy_packet() renders the 33-byte Data Packet (bytes [3..35]) into the
 *  264 pixels that follow.
 *  Total = 24 + 264 = 288 pixels per line. ✓
 */
...
/* Preamble = CS + BS = 3 bytes = 24 pixels (FIXED) */
#define NABTS_PREAMBLE_BYTES  (NABTS_CS_COUNT + 1)   /* 3  */
#define NABTS_FIXED           (NABTS_PREAMBLE_BYTES * 8)  /* 24 pixels */
